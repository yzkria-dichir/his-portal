...
export default function HISDocPortal() {
  ...
  const RenameGroupModal = ({ screenIdx, oldName, onClose }) => {
    const isUngrouped = oldName === "Ungrouped";
    const [val, setVal] = useState(isUngrouped ? "" : oldName);
    const submit = () => {
      const next = (val || "").trim();
      const current = isUngrouped ? "" : oldName;
      if (next === current) { onClose(); return; }
      const s2 = [...screens]; const s = { ...s2[screenIdx] };
      s.fields = (s.fields || []).map(f => ((f.group || "") === current ? { ...f, group: next } : f));
      if (Array.isArray(s.fieldGroups) && !isUngrouped) {
    ...
    return (
      <Modal title={isUngrouped ? "Name Ungrouped Fields" : `Rename Group: ${oldName}`} onClose={onClose}>
        <FormRow label="New Group Name"><Input value={val} onChange={setVal} placeholder={isUngrouped ? "Enter a group name…" : "Enter new group name…"} /></FormRow>
        <p style={{ fontSize: 11.5, color: P.textMuted, margin: "0 0 12px" }}>{isUngrouped ? "All ungrouped fields on this screen will be assigned to this group." : `All fields with group "${oldName}" will be updated.`}</p>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
          <Btn ghost onClick={onClose}>Cancel</Btn>
          <Btn onClick={submit} color={P.green}>Save</Btn>
        </div>
      </Modal>
    );
  };
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: P.bg, color: P.text, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
                          ...
                          {(() => {
                            ...
                            <div key={gi}>
                              <div style={{ display: "flex", alignItems: "center", paddingLeft: 28, gap: 6 }}>
                                ...
                                <button disabled={isFirstGroup} onClick={() => moveGroup(gName, -1)} title="Move group up" style={{ background: isFirstGroup ? "transparent" : "#4ADE8025", border: `1px solid ${isFirstGroup ? "#4ADE8033" : "#4ADE80"}`, color: "#4ADE80", cursor: isFirstGroup ? "default" : "pointer", fontSize: 12, fontWeight: 700, padding: "1px 8px", borderRadius: 4, opacity: isFirstGroup ? 0.35 : 1, fontFamily: "inherit" }}>{"↑"}</button>
                                <button disabled={isLastGroup} onClick={() => moveGroup(gName, 1)} title="Move group down" style={{ background: isLastGroup ? "transparent" : "#60A5FA25", border: `1px solid ${isLastGroup ? "#60A5FA33" : "#60A5FA"}`, color: "#60A5FA", cursor: isLastGroup ? "default" : "pointer", fontSize: 12, fontWeight: 700, padding: "1px 8px", borderRadius: 4, opacity: isLastGroup ? 0.35 : 1, fontFamily: "inherit" }}>{"↓"}</button>
                              ...
                              {fields.map((f, fi) => {
                                ...
                                <div key={fi} style={{ display: "flex", alignItems: "center", paddingLeft: 56, color: "#E0E0E0" }}>
                                  ...
                                  <span style={{ marginLeft: "auto", display: "flex", gap: 2, flexShrink: 0 }}>
                                    <button disabled={isFirst} onClick={() => moveFieldInGroup(f._idx, -1)} title="Move up" style={{ background: "none", border: "none", color: "#4ADE80", cursor: isFirst ? "default" : "pointer", fontSize: 14, fontWeight: 700, padding: "0 4px", opacity: isFirst ? 0.3 : 1 }}>{"\u2191"}</button>
                                    <button disabled={isLast} onClick={() => moveFieldInGroup(f._idx, 1)} title="Move down" style={{ background: "none", border: "none", color: "#60A5FA", cursor: isLast ? "default" : "pointer", fontSize: 14, fontWeight: 700, padding: "0 4px", opacity: isLast ? 0.3 : 1 }}>{"\u2193"}</button>
                        ...
                        return (
                          <div key={gi} style={{ marginBottom: 16 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, paddingBottom: 4, borderBottom: `1px solid ${P.border}` }}>
                              ...
                              <button disabled={isFirstGroup} onClick={() => moveGroup(gName, -1)} title="Move group up" style={{ background: isFirstGroup ? "transparent" : P.greenDim, border: `1px solid ${isFirstGroup ? P.border : P.green}`, color: P.green, cursor: isFirstGroup ? "default" : "pointer", fontSize: 12, fontWeight: 700, padding: "1px 8px", borderRadius: 4, opacity: isFirstGroup ? 0.4 : 1 }}>{"\u2191"}</button>
                              <button disabled={isLastGroup} onClick={() => moveGroup(gName, 1)} title="Move group down" style={{ background: isLastGroup ? "transparent" : P.accentDim, border: `1px solid ${isLastGroup ? P.border : P.accent}`, color: P.accent, cursor: isLastGroup ? "default" : "pointer", fontSize: 12, fontWeight: 700, padding: "1px 8px", borderRadius: 4, opacity: isLastGroup ? 0.4 : 1 }}>{"\u2193"}</button>
                                ...
                                <tbody>{fields.map((f, fi) => {
                                  ...
                                  <tr key={fi} style={{ borderBottom: `1px solid ${P.border}15` }}>
                                    ...
                                    <TD><div style={{ display: "flex", gap: 4 }}>
                                      <button disabled={isFirst} onClick={() => moveFieldInGroup(f._idx, -1)} title="Move up" style={{ background: isFirst ? "transparent" : P.greenDim, border: `1px solid ${isFirst ? P.border : P.green}`, borderRadius: 6, color: P.green, cursor: isFirst ? "default" : "pointer", fontSize: 12, fontWeight: 700, padding: "2px 7px", opacity: isFirst ? 0.35 : 1 }}>{"\u2191"}</button>
                                      <button disabled={isLast} onClick={() => moveFieldInGroup(f._idx, 1)} title="Move down" style={{ background: isLast ? "transparent" : P.accentDim, border: `1px solid ${isLast ? P.border : P.accent}`, borderRadius: 6, color: P.accent, cursor: isLast ? "default" : "pointer", fontSize: 12, fontWeight: 700, padding: "2px 7px", opacity: isLast ? 0.35 : 1 }}>{"\u2193"}</button>
      ...
      {modal?.type === "renameGroup" && <RenameGroupModal screenIdx={modal.screenIdx} oldName={modal.oldName} onClose={() => setModal(null)} />}
